...
/* ------- ключ платформы, разобранный на вопросы -------

   `linux-chromium-1x-390x844` — это адрес каталога, и как адрес он безупречен.
   Как переключатель — нет: в нём склеены четыре независимых ответа (машина,
   движок, плотность пикселей, размер окна), а спрашивают их по одному.
   «Как это выглядит в firefox» и «как это выглядит на телефоне» — разные
   вопросы, и один список из шести-восемнадцати ключей заставляет отвечать на
   оба сразу, читая строку по буквам.

   Разбор по ФОРМЕ, а не по списку известных браузеров: ключ мог быть записан
   версией, которая знала другой их набор, и отвечать «не знаю» на собственные
   данные — худшее из поведений. Не разобрался — `parsed:false`, и вызывающий
   показывает ключ как есть. Это всегда правда.

   Базовый размер окна суффикса не имеет намеренно (см. `vistest/matrix.py`):
   под ключом без суффикса лежат все эталоны, снятые до появления матрицы, и
   переезд обесценил бы их разом. Поэтому пустой `viewport` — это не «размер
   неизвестен», а «базовый размер», и подписывать его надо словом, а не
   прочерком. */
function parsePlatform(key){
  const raw=String(key||'');
  const m=raw.match(/^([^-]+)-([^-]+)-([\d.]+)x(?:-(\d{2,5})x(\d{2,5}))?$/);
  if(!m)return {raw,parsed:false,os:'',browser:'',scale:'',
                viewport:'',w:0,h:0};
  return {raw,parsed:true,os:m[1],browser:m[2],scale:m[3]+'x',
          viewport:m[4]?m[4]+'x'+m[5]:'',
          w:Number(m[4]||0),h:Number(m[5]||0)};
}

/* Подпись размера окна. Пустой — базовый: так он и называется в конфиге. */
function viewportLabel(vp,base){
  if(vp)return vp.replace('x','\u00d7');
  return base?base.replace('x','\u00d7'):'base size';
}

/* Во что этот размер окна складывается на глаз. Число «390x844» ничего не
   говорит человеку, который держит в голове «телефон / планшет / десктоп», —
   а решение «а этот вариант мне вообще нужен» принимается именно в этих
   словах. Границы взяты по общему месту (медиазапросы Bootstrap/Tailwind), и
   спорить о них не стоит: подпись вспомогательная, точное число рядом. */
function viewportKind(w){
  if(!w)return '';
  if(w<=520)return 'phone';
  if(w<=900)return 'tablet';
  if(w<=1500)return 'laptop';
  return 'desktop';
}

/* Метка движка. Монохромная и рисованная разметкой, а не картинкой: цвет в
   этом интерфейсе значит «требует ответа», и фирменные цвета браузеров сломали
   бы этот словарь на самом заметном месте экрана. Незнакомый движок получает
   пустой круг — честнее, чем чужая иконка. */
const BROWSER_MARK={
  chromium:'<circle cx="8" cy="8" r="6"/><circle cx="8" cy="8" r="2.4"/>'
    +'<path d="M8 5.6H14M8 5.6L5 10.8M8 10.4L5 5.2"/>',
  chrome:'<circle cx="8" cy="8" r="6"/><circle cx="8" cy="8" r="2.4"/>'
    +'<path d="M8 5.6H14M8 5.6L5 10.8M8 10.4L5 5.2"/>',
  firefox:'<circle cx="8" cy="8" r="6"/>'
    +'<path d="M11.4 5.2c-2.6-1.3-5.2-.2-5.9 1.9-.5 1.7.5 3.2 2.2 3.2 1.3 0 2-.8 2-1.6"/>',
  webkit:'<circle cx="8" cy="8" r="6"/><path d="M10.7 5.3L6.9 6.9 5.3 10.7l3.8-1.6z"/>',
  safari:'<circle cx="8" cy="8" r="6"/><path d="M10.7 5.3L6.9 6.9 5.3 10.7l3.8-1.6z"/>',
};
function browserMark(name){
  const body=BROWSER_MARK[String(name||'').toLowerCase()]||'<circle cx="8" cy="8" r="6"/>';
